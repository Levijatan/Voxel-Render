...
pub struct PointCloud {
    ...
    remove_sun_light_queue: Vec<LightRemovalNode>,
}
...
impl PointCloud {
    pub fn new(chunk_size: usize) -> PointCloud {
        return PointCloud {
            ...
            remove_sun_light_queue: Vec::new(),
        };
    }

    pub fn insert_chunk(&mut self, key: ChunkKey, chunk: Chunk, reg: &VoxelReg) -> ChunkKey {
        self.c.insert(key, chunk);
        if !self.keys.contains(&key) {
            self.keys.push(key);
            //self.init_sunlight(key, reg);
        }
        return key;
    }
    ...
    pub fn init_sunlight(&mut self, key: ChunkKey, reg: &VoxelReg) {
        println!("init sun at {}", key);
        let c = self.c.get_mut(&key).unwrap();
        let top_pos = c.pos + glm::make_vec3(&NORMALS[2]);
        println!("pos: {:?} top pos: {:?}", c.pos, top_pos);
    ...
    pub fn add_sunlight(&mut self, reg: &VoxelReg) {
        while !self.sun_light_queue.is_empty() {
            let node = self.sun_light_queue.pop().unwrap();
            let lightlevel = self.c[&node.chunk].sunlight_at(node.idx);

            for n in &NORMALS {
                let norm = glm::make_vec3(n);
                let n_pos = node.world_pos + norm;
                let key = self.voxel_to_chunk_key(n_pos);
                if self.keys.contains(&key) && self.c[&key].check_voxel_transparency(n_pos, reg) {
                    let idx = self.c[&key].calc_idx_world(n_pos);
                    if self.c[&key].sunlight_at(idx) + 1 < lightlevel {
                        if norm.y != -1.0 {
                            self.c
                                .get_mut(&key)
                                .unwrap()
                                .set_sunlight_at(idx, lightlevel - 1);
                        } else {
                            self.c
                                .get_mut(&key)
                                .unwrap()
                                .set_sunlight_at(idx, lightlevel);
                        }
                        self.sun_light_queue.push(LightNode {
                            world_pos: n_pos,
                            idx,
    ...
    pub fn remove_sunlight(&mut self) {
        while !self.remove_sun_light_queue.is_empty() {
            let node = self.remove_sun_light_queue.pop().unwrap();
            ...
            for n in &NORMALS {
                ...
                if self.keys.contains(&key) {
                    ...
                    if n_level != 0
                        && (n_level < lightlevel || (lightlevel == MAX_LIGHT && norm.y == -1.0))
                    {
                        self.remove_sunlight_at(n_pos);
    ...
    pub fn add_torch_lighting(&mut self, reg: &VoxelReg) {
        while !self.light_queue.is_empty() {
            ...
            for n in &NORMALS {
                ...
                if self.keys.contains(&key) && self.c[&key].check_voxel_transparency(n_pos, reg) {
                    let idx = self.c[&key].calc_idx_point(n_pos);
    ...
    pub fn update_lighting(&mut self, reg: &VoxelReg) {
        self.remove_sunlight();
        self.add_sunlight(reg);
        self.remove_torch_lighting();
        self.add_torch_lighting(reg);
    }

    pub fn voxel_to_chunk_key(&self, pos: Vec3) -> ChunkKey {
        ChunkKey::new(util::voxel_to_chunk_pos(pos, self.chunk_size))
    }

    pub fn update(&mut self, reg: &VoxelReg) {
        for key in self.keys.iter() {
            if self.c.contains_key(key) && self.c[key].update {
                for idx in 0..self.c[key].tot_size {
                    let vox_chunk_pos = util::idx_to_pos(idx, self.chunk_size);
                    ...
                    if !vox_attr.transparent {
                        for n in &NORMALS {
                            ...
                            if self.c[key].in_chunk(voxel_neigh) {
                                render = self.c[key].check_voxel_transparency(voxel_neigh, reg);
                            } else {
                                let chunk_neigh = self.c[key].pos + norm;
                                let k = ChunkKey::new(chunk_neigh);

                                if self.c.contains_key(&k) {
                                    render = self.c[&k].check_voxel_transparency(voxel_neigh, reg);
                                } else if norm.y == 1.0 {
                                    render = true;
                                }
                            }
    ...
    pub fn create_cube(&mut self, start: Vec3, stop: Vec3, reg: &VoxelReg) {
        let (t_start, t_stop) = util::check_start_stop(start, stop);

        let voxel_type = reg.key_from_string_id(OPAQUE_VOXEL);
        for x in t_start.x as i32..t_stop.x as i32 {
            for y in t_start.y as i32..t_stop.y as i32 {
                for z in t_start.z as i32..t_stop.z as i32 {
                    self.set_voxel(voxel_type, Vec3::new(x as f32, y as f32, z as f32), reg);
                }
            }
        }
    }

    pub fn remove_torchlight_at(&mut self, pos: Vec3) {
    ...
    pub fn remove_sunlight_at(&mut self, pos: Vec3) {
        let key = self.voxel_to_chunk_key(pos);
        if self.c.contains_key(&key) {
            let idx = self.c[&key].calc_idx_world(pos);
            let val = self.c[&key].sunlight_at(idx);
            self.remove_sun_light_queue.push(LightRemovalNode {
                world_pos: pos,
                idx,
                chunk: key,
                lightlevel: val,
            });
            self.c.get_mut(&key).unwrap().set_sunlight_at(idx, 0);
        }
    }

    pub fn set_voxel(&mut self, v: u64, pos: Vec3, reg: &VoxelReg) {
        let chunk_pos = util::voxel_to_chunk_pos(pos, self.chunk_size);
        let key = self.new_chunk_point(chunk_pos, reg);
        let attr = reg.voxel_attributes(&v);
        if !attr.transparent {
            self.remove_sunlight_at(pos);
        }
